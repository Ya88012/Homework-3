...
class MyPortfolio:
    ...
    def calculate_weights(self):
        ...
        for i in range(start_day, total_len):
            ...
            if i % 10 == 0 or i == start_day:
                sol = self.my_strategy(prices, no_short = False)
                # Normalize
                sol = sol / sol.sum()
            # Update the answer
    ...
    def my_strategy(self, prices, no_short = True):
        '''
        Ref:
        https://medium.com/@carlolepelaars/towards-a-more-profitable-s-p500-with-portfolio-optimization-c43a37d9078f
        '''
        mu = prices.mean()
        sigma = prices.cov()
